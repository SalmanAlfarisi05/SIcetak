<x-app-layout>
    <div class="bg-white rounded-lg p-5 shadow border space-y-5 max-w-3xl mx-auto">
        ...
        <form action="{{ route('laminasi.store') }}" method="POST">
            @csrf
            @method('POST')
        <div class="space-y-3">
            <div class="flex flex-col gap-1">
                <label for="nama_laminasi">nama Laminasi</label>
                <input required type="text" id="nama_laminasi" name="nama_laminasi" class="rounded-lg" placeholder="Masukkan Nama Laminasi">
            </div>
            <div class="flex flex-col gap-1">
